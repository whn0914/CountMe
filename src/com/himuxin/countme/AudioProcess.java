...
public class AudioProcess {
	...
	//启动程序
	public void start(AudioRecord audioRecord, int minBufferSize, Handler handler, Runnable callback, Runnable callback_log) {
		isRecording = true;
		result[0] = false;
		record_thread = new RecordThread(audioRecord, minBufferSize, handler, callback, callback_log);
	...
	class RecordThread extends Thread{
		...
		public RecordThread(AudioRecord audioRecord,int minBufferSize, Handler handler, Runnable callback, Runnable callback_log){
...
//					fos.write('\n');
					...
					if(index>=0 && result[index]==false) {	// 第一次出现某频率
						...
						time_stamps[index] = tsLong.toString();	// get the time stamp
						result[index] = true;
						log = "频率：" + freq + "加入网络";
						...
						if(Long.valueOf(ts_current) - Long.valueOf(time_stamps[index]) >= 30) {	// 距离上一次接收超过30s
